...
class Dataset_Return_Four(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_blue_LL = np.uint8(cv2.resize(image_blue_LL,(256,256),interpolation=cv2.INTER_CUBIC))
        image_green_LL = np.uint8(cv2.resize(image_green_LL,(256,256),interpolation=cv2.INTER_CUBIC))
        image_red_LL = np.uint8(cv2.resize(image_red_LL,(256,256),interpolation=cv2.INTER_CUBIC))
        
        image_blue_LL = np.expand_dims(image_blue_LL, axis=-1)
        image_green_LL = np.expand_dims(image_green_LL, axis=-1)
        image_red_LL = np.expand_dims(image_red_LL, axis=-1)
        
        mask_and_blue = np.bitwise_and(image_blue_LL, mask)
        val = np.max(mask_and_blue) * self.threshold
        mask_and_blue[mask_and_blue <= val] = 0
        mask_and_blue[mask_and_blue > val] = 255

        mask_and_green = np.bitwise_and(image_green_LL, mask)
        val = np.max(mask_and_green) * self.threshold
        mask_and_green[mask_and_green <= val] = 0
        mask_and_green[mask_and_green > val] = 255

        mask_and_red = np.bitwise_and(image_red_LL, mask)
        val = np.max(mask_and_red) * self.threshold
        mask_and_red[mask_and_red <= val] = 0
        mask_and_red[mask_and_red > val] = 255

        # image_blue = np.expand_dims(image_blue, axis=-1)
        # image_green = np.expand_dims(image_green, axis=-1)
        # image_red = np.expand_dims(image_red, axis=-1)
        ...
        image_blue_LL = self.preprocess_input(image_blue_LL)
        image_green_LL = self.preprocess_input(image_green_LL)
        image_red_LL = self.preprocess_input(image_red_LL)
        image_gray = self.preprocess_input(image_gray)

        image_blue_LL = torch.from_numpy(image_blue_LL).permute(2, 0, 1).float()
        mask_and_blue = torch.from_numpy(mask_and_blue).permute(2, 0, 1).float() / 255.

        image_green_LL = torch.from_numpy(image_green_LL).permute(2, 0, 1).float()
        mask_and_green = torch.from_numpy(mask_and_green).permute(2, 0, 1).float() / 255.
        
        image_red_LL = torch.from_numpy(image_red_LL).permute(2, 0, 1).float()
        ...
        return (image_blue_LL, image_green_LL, image_red_LL, image_gray), (mask_and_blue, mask_and_green, mask_and_red, mask)
